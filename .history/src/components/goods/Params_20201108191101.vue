                ...
                <el-table :data="paramsData.many" border stripe>
                    <el-table-column type="expand">
                        <template slot-scope="scope">
                            <el-tag type="primary" v-for="(item,index) in attrTags(scope.row.attr_vals)" :key="index" closable style="margin-left:10px;" @close="deleteTag(scope.row,item)">{{item}}</el-tag>
                            <el-input
                            class="input-new-tag"
    ...
    methods:{
        ...
        async handleInputConfirm(row){
            if(this.newTagValue.trim().length==0){
                ...
                row.attr_vals += row.attr_vals==''?this.newTagValue:` ${this.newTagValue}`
                const {data:res} = await this.$http.put(`categories/${this.selectedCateKeys[this.selectedCateKeys.length-1]}/attributes/${row.attr_id}`,{
        ...
        deleteTag(row,item){
            this.$confirm('此操作将永久删除该选线, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(async () => {
                const {data:res} = this.$http.put(`categories/${this.selectedCateKeys[this.selectedCateKeys.length-1]}/attributes/${row.attr_id}`,{
                    attr_name:row.attr_name,
                    attr_sel:this.tabsActiveName,
                    attr_vals:row.attr_vals
                })
                console.log(res)
                row.attr_vals =  row.attr_vals.replace(item,'').trim()
            }).catch(() => {
                this.$message({
                    type: 'info',
                    message: '已取消删除'
