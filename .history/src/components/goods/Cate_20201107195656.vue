...
export default {
    ...
    methods:{
        ...
        setNewName(cat_id,newName,arr){
            ...
            arr.forEach(item=>{
                if(item.children){
                    this.setNewName(cat_id,newName,item.children)
                }
            })
        },
        deleteByCateList(id,arr){
            let index = arr.findIndex(item=>item.cat_id==id)
            if(index!=-1) return arr.splice(index,1)
            arr.forEach(item=>{
                if(item.children){
                    this.deleteByCateList(id,item.children)
                }
            })
        },
        deleteCate(row){
            this.$confirm('此操作将永久删除该分类, 是否继续?', '提示', {
                ...
                if(res.meta.status!=200) return this.$message.error(res.meta.msg);
                this.$message.success(res.meta.msg)
                this.deleteByCateList(row.cat_id,this.cateList)
            }).catch(() => {
                this.$message({
                    type: 'info',
                    message: '已取消删除'
